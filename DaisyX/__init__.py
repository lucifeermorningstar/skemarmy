...
def get_translation(transKey, params: list = None):
    ret = _tr.get_translation(DAISY_LANG, transKey)
...
DAISY_LANG = environ.get('DAISY_LANG', 'en')
...
BOT_VERSION = '0.0.1 Beta'
SUPPORT_GROUP = 'DaisySupport_Official'
CHANNEL = 'DaisyXUpdates'
...
# DaisyX Session
SESSION = environ.get('SESSION', 'DaisyX')

# SedenBot repo url for updater
REPO_URL = environ.get('REPO_URL', 'https://github.com/lucifeermorningstar/SkemArmy')
...
def __import_modules():
    ...
    for module in modules:
        try:
            ...
            import_module(f'DaisyX.modules.{module}')
        except Exception:
            if LOG_VERBOSE:
...
LOGS.info(get_translation('DaisyXVersion', [BOT_VERSION]))
